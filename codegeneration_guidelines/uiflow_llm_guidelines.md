...
5. `eventChannels` is captured once on first render via `useRef`; later prop changes are ignored.
6. Channel subscriptions are installed once on mount (`useEffect(..., [])`).
7. Any channel `emit` triggers FlowRunner re-render.
8. `initialData` is shallow-copied once at initialization (`data: { ...initialData }`).
9. Data is mutable inside steps; transitions force re-render by cloning `data` reference.
10. If `onOutput` returns an unknown step name or `void`, FlowRunner stays on current step and re-renders.
11. Step errors are logged (`console.error`) and not rethrown.
12. Action steps render `"Processing..."` while busy.
...
10. Keep `eventChannels` object identity stable after mount.

## 6) Channel lifecycle pattern (important)

Because FlowRunner captures `eventChannels` once, unstable channel creation causes bugs.
...
<FlowRunner eventChannels={{ studiedCounter }} ... /> // new object each render
```
...
6. Assuming `eventChannels` prop updates are observed after mount.
